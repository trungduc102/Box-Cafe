...
public class Chart extends javax.swing.JPanel {
    ...
    public void setLegend(String name) {
        for (Component component : panelLegend.getComponents()) {
            if (component instanceof LegendItem) {
                LegendItem item = (LegendItem) component;
                if (name.equals("")) {
                    item.getLbName().setText("Chưa có dữ liệu");
                }
                item.getLbName().setText(name);
            }
        }
        panelLegend.repaint();
        panelLegend.revalidate();
    ...
    public void clear() {
        ...
        blankPlotChart.initValues(0, 10);
        model.clear();
        repaint();
    }
